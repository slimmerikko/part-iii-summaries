...
\begin{solution}
	...
	% \item From Fourier theory it is known that $\qty{s_n(x) = \sqrt 2\sin(nx) : n \in \NN}$ and $\qty{1} \cup \qty{c_n(x) = \sqrt 2\cos(nx) \mid n \in \NN}$ are orthonormal bases of $L^2[0, 1]$. 
	%Note that 
	%\[
	%(A s_n)(y) = \int_0^y s_n(x) \dd{x} = \sqrt 2 \int_0^y \sin(nx) \dd{x} = \frac{\sqrt 2}{n} \qty(1 - \cos(ny)) = \frac{\sqrt 2}{n} c_0(y) - \frac1n c_n(y) 
	%\]
